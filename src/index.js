import { readJSON, readdir, exists as _exists, readdirSync, statSync } from "fs-extra";
import sharp from "sharp";

export default class ResponsiveJSONWebpackPlugin {
    constructor(
        { 
            dataPath = "data", 
            imagePath = "images", 
    ...
    processDataFolders(dataFolders) {
        return Promise.all(dataFolders.map(folder =>
            readdir(`${this.dirs.sourceTemplates}/${folder}/${this.dirs.dataPath}`)
    ...
    processDataFiles(folder, dataFiles) {
        return Promise.all(dataFiles.map(file =>
            readJSON(`${this.dirs.sourceTemplates}/${folder}/${this.dirs.dataPath}/${file}`)
                .then(data => _exists(`${this.dirs.sourceTemplates}/${folder}/${this.dirs.imagePath}/${file}`)
                    .then(exists => {
                        if(exists){
                            return readJSON(`${this.dirs.sourceTemplates}/${folder}/${this.dirs.imagePath}/${file}`)
    ...
    getChangedDependencies(compilation) {
        ...
        fileDependencies.forEach(rawFileName => {
            ...
            const time = statSync(rawFileName).mtime.getTime()
