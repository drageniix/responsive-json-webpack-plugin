...
describe('create portion pictures', () => {
    ...
    rjInstance.createImg.mockReturnValue(
        Promise.resolve({
            src: 'test.src',
            size: 16
        })
    ...
    rjInstance.createImgResolutions.mockReturnValue(
        Promise.resolve({
            sizes: '20w',
            srcset: [{}]
        })
    ...
    rjInstance.createPictureSources.mockReturnValue(
        Promise.resolve({
            media: 'media',
            sizes: '20w',
            srcset: [{}]
        ...
        return rjInstance
            ...
            .then(result =>
                expect(result).toEqual({
                    size: 16,
                    sizes: '20w',
                    sources: { media: 'media', sizes: '20w', srcset: [{}] },
                    src: 'test.src',
                    srcset: [{}]
                })
            );
...
test('inject', () => {
    ...
    rjInstance.createPortionPictures.mockReturnValue(
        Promise.resolve({
            src: 'test.src'
        })
    );
