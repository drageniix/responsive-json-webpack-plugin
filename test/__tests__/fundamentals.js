...
describe("save to assets", () => {
    test("save json", () => {
        ...
        expect(rjInstance.assets[expectedPath].source()).toEqual(Buffer.from(stringData))
...
describe("generation of file names", () => {
    test("valid input", () => {
        expect(rjInstance.generateFileName(
            {
                name: "abc",
                index: 7,
                size: 16,
    ...
    test("strip invalid characters", () => {
        expect(rjInstance.generateFileName(
            ...
            "potato[name]@$[index]<[size]")
        ).toBe("examples/output/images/potato#23/1.png")
    })
...
describe("insertion of json", () => {
    ...
    test("array", () => {
        const nestingArr = { arr: [{ name: "one" }, { name: "two" }, { name: "three" } ] }
