...
var makeImageSrcSet = function (srcSets, imagePath) {
    return srcSets
        .map(function (img) { return "" + imagePath + img.src + " " + img.size + "w"; })
        .join(", ");
};
var ResponsiveImage = function (props) {
    ...
    return image.sources && window && !!window.HTMLPictureElement ? (React.createElement("picture", __assign({}, other),
        image.sources &&
            image.sources.map(function (source, index) { return (React.createElement("source", { key: index, srcSet: source.srcset && makeImageSrcSet(source.srcset, imagePath), sizes: source.sizes, media: source.media })); }),
